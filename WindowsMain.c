/*To compile, refer to the README.txt*/
...
int badai(int l_board[], int depth){                    //uses current boardstate to find the best possible moves
    int legalMovesArr[9];                               //creates an array to correspond if the spot in the board is empty or filled
    legalMoves(board, legalMovesArr);                   //fills the legalmovesarray with 1 or 0's and checks with the current board state whether if it is already filled or not
    int val = -100;                                     //set the initial best move to -100
    int worst_move = 0;                                 //set local value to 0 to avoid garbage values
    for (int i = 0; i < 9; i++){
    ...
    return worst_move;                                  //the closer val is to -10, the worse the move
}
    ...
    if (mode == 1) {
        ...
        if (gs == X){
            ...
            if (turn == O){
                ...
                else if(difficulty == 2 ){ //lvl 1
                    putInBoard(board, badai(board, 4), X);
                    gameLogic(board, mode);
                }
            
                else if(difficulty == 3){  //lvl 2
